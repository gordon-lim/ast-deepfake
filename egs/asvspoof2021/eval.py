...
def evaluate(audio_model_path, eval_loader):
    ...
    ast_mdl = ASTModel(label_dim=2, input_tdim=input_tdim, imagenet_pretrain=True, audioset_pretrain=True)
    ...
    avg_recall = recall_score(targets, binary_predictions, average="binary")
...
def main():
    base = os.getcwd()
    data_path = os.path.join(base, 'datafiles', 'la_train_fold_1.csv')
